...
以下数据来自 4× CMP 170HX 实机（PP4、512k 上下文、DSpark 投机解码 n=3、FULL CUDA graph）：

| 负载 | 结果 |
|---|---|
| 单流解码（浅层） | 49 tok/s |
| 单流解码（85k–366k 深度） | **88–92 tok/s — 与深度无关** |
| 首字延迟 (TTFT) | 1k=1.1s / 85k=16s / 366k=94s（prefill ~4k tok/s） |
| Agent 并发（3.2k 共享前缀） | C8=379 / **C16=497 峰值** / C32=404 tok/s 聚合 |
